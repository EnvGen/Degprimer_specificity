...
def compare_lower(opt1, opt2):
    result=float(opt2)
    if float(opt1) < result:
        result = float(opt1)
    return result

def compare_bigger(opt1, opt2):
    result=float(opt2)
    if float(opt1) > result:
        result = float(opt1)
    return result
...
def store_max_min(mylist1, mylist2):
    j=0
    mylist=[mylist1[13], mylist1[12], mylist1[2], mylist1[4]]
    for opt, opt2 in zip(mylist, mylist2[:4]):
        mylist2[j]=compare_lower(float(opt), float(opt2))
        j += 1

    for opta, optb in zip(mylist, mylist2[4:]):
        mylist2[j]=compare_bigger(float(opta), float(optb))
        j += 1

    return mylist2

def build_dict(key, mydict):
    g = key.split("_")[0]
    sp = key.split("_")[1]
    st = "_".join(key.split("_")[2:])
    if g in mydict:
        if sp in mydict[g].keys():
            if st in mydict[g][sp].keys():
                mydict[g][sp][st] += 1
            else:
                mydict[g][sp][st] = 1
        else:
...
def select_hits_and_printing_out_results(file_out, dict_id_hit):
    ...
    with open(file_out + "_selected", "w") as fout, open(file_out, "w") as fout2, open("Summary_table", "a") as filetable:
        print("# {}".format("\t".join(header)), file=fout)
        print("# {}".format("\t".join(header)), file=fout2)
        Genus, Genus_slc, mydict_selected=get_hits_by_genus(dict_id_hit, fout, fout2)
        ...
        print("## Total number of unique hits above threshold (% identity >= {} - % Query_cov >= {}): {}".format(args.d, args.l, len(mydict_selected.keys())), file=fout)
    ...
    return mydict_selected, t_genus,t_spp, genome_counter_slc
...
id_hit_selected, t_genus,t_spp, genome_counter_slc = select_hits_and_printing_out_results(args.out, id_hit)
